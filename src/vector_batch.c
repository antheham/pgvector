...
batch_l2_distance(PG_FUNCTION_ARGS)
{
	...
	if (cuda_is_available()) {
		...
		/* 准备批量查询数据 */
		float *batch_query_data = (float *) palloc(queries->count * queries->dim * sizeof(float));
		float *target_data = (float *) palloc(queries->dim * sizeof(float));
		float *gpu_distances = (float *) palloc(queries->count * sizeof(float));
		...
		/* 创建GPU上下文 - 使用批量支持 */
		CudaCenterSearchContext* ctx = cuda_center_search_init(queries->count, queries->dim, false);
		if (ctx) {
