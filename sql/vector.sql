...
-- 批量扫描函数
CREATE FUNCTION ivfflat_batch_scan_create(
    index_oid oid,
    nkeys integer,
    norderbys integer,
    batch_keys internal
) RETURNS internal
AS 'MODULE_PATHNAME', 'ivfflat_batch_scan_create'
LANGUAGE C STRICT;


CREATE FUNCTION ivfflat_batch_scan_end(
    scan internal
) RETURNS void
AS 'MODULE_PATHNAME', 'ivfflat_batch_scan_end'
LANGUAGE C STRICT;

-- 获取批量扫描结果（包含距离信息）
CREATE FUNCTION ivfflat_batch_scan_get_results(
    scan internal,
    direction integer,
    k integer
) RETURNS TABLE(
...
AS 'MODULE_PATHNAME', 'ivfflat_batch_scan_get_results'
LANGUAGE C STRICT;

-- 批量扫描键管理函数
-- 创建一个包装函数，接受 internal 参数
CREATE FUNCTION scanbatch_create_wrapper(
    dummy internal,
    nkeys integer,
    vec_dim integer
) RETURNS internal
AS 'vector', 'scanbatch_create_wrapper'
LANGUAGE C STRICT IMMUTABLE;

CREATE FUNCTION scanbatch_add_vector(
    batch internal,
    index integer,
    vector vector
) RETURNS void
AS 'vector', 'scanbatch_add_vector'
LANGUAGE C STRICT;

CREATE FUNCTION scanbatch_free(
    batch internal
) RETURNS void
AS 'vector', 'scanbatch_free'
LANGUAGE C STRICT;
